...
\newpage
\lhead{Deadline: 2025-02-05, 23:59}
...
\begin{enumerate}
    ...
    % \item Check order and rank conditions in the case $\gamma_{33} = 0$.
\end{enumerate}

% \item Consider the system of simulteneous equations
% \[
% \begin{cases}
% a_t = \gamma b_t + \beta_1 x_t + \beta_2 v_t + \beta_3 w_t + u_{1t}, \\
% b_t = \gamma a_t  + \delta_1 x_t + \delta_2 v_t  + u_{2t}.
% \end{cases}
% \]
% We assume that observations are independent, $a_t$ and $b_t$ are endogenous and $x_t$, $v_t$ and $w_t$ are exogeneous.

% \begin{enumerate}
%     \item Check the order condition for each equation. 
%     \item Check the rank condition for each equation.
%     \item What exactly will happen if naive John will try to use 2SLS method for the first equation?
% \end{enumerate}
...
Made in Moscow and Belgrad with love for pandas $\heartsuit$

Deadline: 2025-02-23, 23:59.

\begin{enumerate}
\item Let $x_i$ be the number coffee cups drinken before econometrics exam by $i$-th student. 
The dummy variable $d_i$ is equal to one for those students who praise hedgehogs and dependen variable $y_i$ is equal to one for those who pass the econometrics exam. 

Based on $n=1000$ observation Nikola Tesla estimated the logistic model:
\[
\hat\P(y_i = 1 \mid x_i, d_i) = \Lambda(0.2 + 0.1 x_i + 0.15 d_i).
\]

Alice has drinken $3$ cups of coffee and does not praise hedgehogs.

\begin{enumerate}
    \item Estimate the probability and odds of passing an exam for Alice.
    \item Estimate the marginal effect of the number of coffee cups on probability for Alice.
    \item Estimate the partial effect of increasing the number of coffee cups by one for Alice. 
    Compare it with the marginal effect.
    \item Estimate the odds ratio (praising hedgejogs to non-praising hedgehogs) for Alice. 
\end{enumerate}

\item You have a toy dataset:

\begin{tabular}{*{3}{c}}
    \toprule
    value of $y_i$ & value of $x_i$ & number of observations \\
    \midrule
    $y_i = 0$ & $x_i = 3$ &  200  \\
    $y_i = 0$ & $x_i = 5$ &  300  \\
    $y_i = 1$ & $x_i = 3$ &  400  \\
...
Estimate the logistic model $\P(y_i = 1 \mid x_i) = \Lambda(\beta_1 + \beta_2 x_i)$ using maximum likelihood by bare hands.


\item Solve the practice problem: 

\url{https://colab.research.google.com/drive/1_WCf2Emz3qJwqElnBKF6kmnTzJ7-OJ1P?usp=sharing}

Use the preprocessed dataset: 

\url{https://github.com/bdemeshev/hse_panda_metrics_2024_2025/blob/main/data}
